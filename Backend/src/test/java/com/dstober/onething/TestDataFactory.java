...
/**
 * Factory class for creating test data objects. Provides default values for all required fields.
 */
public class TestDataFactory {
  ...
  public static Task createTaskWithFrequency(Frequency frequency, Instant lastCompleted) {
    Task task = createTask();
    task.setFrequency(frequency);
    task.setLastCompleted(lastCompleted);
    return task;
  }
  ...
  public static TaskCreateRequest createTaskCreateRequest(String name, Frequency frequency) {
    return new TaskCreateRequest(
        name, DEFAULT_CATEGORY_ID, TimeBracket.FIFTEEN_TO_THIRTY, frequency, null);
  }
  ...
  public static org.springframework.security.core.Authentication createAuthentication(Long userId) {
    com.dstober.onething.security.UserPrincipal userPrincipal =
        createUserPrincipal(userId, DEFAULT_EMAIL);
    return new org.springframework.security.authentication.UsernamePasswordAuthenticationToken(
        userPrincipal, null, userPrincipal.getAuthorities());
  }
}
